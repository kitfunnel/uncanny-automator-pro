...
class Licensing {
	...
	public function edd_activate_license( $license_key ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			$error = __( 'An error occurred, please try again.', 'uncanny-automator-pro' );
	...
	public function save_license_key( $license_key ) {
		...
		return update_option( 'uap_automator_pro_license_key', $license_key );
	}
	...
	public function maybe_pre_activate() {
		...
		if ( 'not-exists' !== get_option( 'uap_automator_pro_license_key', 'not-exists' ) ) {
			return;
		}
