...
class Activity_Log_Settings {
	...
	public function delete_old_logs() {
		...
		$purge_days_limit = apply_filters(
			'automator_pro_auto_purge_logs_days',
			get_option( 'uap_automator_purge_days', '' )
		);

		if ( false === apply_filters( 'automator_pro_auto_purge_logs_force_delete', false, $purge_days_limit ) ) {
		...
		$this->delete_records( $purge_days_limit );
	}

	/**
	 * @param $purge_days_limit
	 *
	 * @return void
	 */
	public function delete_records( $purge_days_limit = 1 ) {
		global $wpdb;
		...
		$previous_time = apply_filters( 'automator_pro_auto_purge_logs_previous_time', $previous_time, $purge_days_limit );

		$qry = $wpdb->prepare(
			"SELECT `ID`, `automator_recipe_id` FROM {$wpdb->prefix}uap_recipe_log WHERE `date_time` < %s AND `completed` IN (" . join( ',', $finished_statuses ) . ')',
			$previous_time
		);
