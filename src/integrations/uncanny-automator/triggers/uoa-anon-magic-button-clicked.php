...
/**
 * Class UOA_ANON_MAGIC_BUTTON_CLICKED
...
class UOA_ANON_MAGIC_BUTTON_CLICKED {
	...
	public function __construct() {
		$this->trigger_code = 'ANONWPMAGICBUTTON';
	...
	public function define_trigger() {
		$trigger = array(
			...
			'sentence'            => sprintf( __( ' %1$s is clicked', 'uncanny-automator-pro' ), '[automator_button id="{{id:ANONWPMAGICBUTTON}}" label="' . __( 'Click here', 'uncanny-automator-pro' ) . '"]' ),
			/* translators: Logged-in trigger - WordPress Core */
			'select_option_name'  => __( '{{A magic button}} is clicked', 'uncanny-automator-pro' ),
		...
		if ( $results ) {
			foreach ( $results as $result ) {
				if ( true === $result['result'] && $result['args']['trigger_id'] && $result['args']['trigger_log_id'] ) {
					...
					if ( ! empty( $_POST ) && isset( $_POST['automator_button_post_id'] ) ) {
						$save_meta['meta_key']   = 'automator_button_post_id';
						$save_meta['meta_value'] = Automator()->uap_sanitize( $_POST['automator_button_post_id'] );
						Automator()->insert_trigger_meta( $save_meta );
					}

					if ( ! empty( $_POST ) && isset( $_POST['automator_button_post_title'] ) ) {
						$save_meta['meta_key']   = 'automator_button_post_title';
						$save_meta['meta_value'] = Automator()->uap_sanitize( $_POST['automator_button_post_title'] );
						Automator()->insert_trigger_meta( $save_meta );

						$save_meta['meta_key']   = 'automator_button_post_url';
						$save_meta['meta_value'] = Automator()->uap_sanitize( get_permalink( $_POST['automator_button_post_id'] ) );
						Automator()->insert_trigger_meta( $save_meta );
					}
