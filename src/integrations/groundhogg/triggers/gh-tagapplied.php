...
class GH_TAGAPPLIED {
	...
	public function __construct() {
		$this->trigger_code = 'GHTAGAPPLIED';
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			'sentence'            => sprintf( esc_attr__( '{{A tag:%1$s}} is added to a user', 'uncanny-automator-pro' ), $this->trigger_meta ),
			/* translators: Logged-in trigger - Groundhogg */
			'select_option_name'  => esc_attr__( '{{A tag}} is added to a user', 'uncanny-automator-pro' ),
			'action'              => 'groundhogg/contact/tag_applied',
			'priority'            => 10,
			'accepted_args'       => 2,
			'validation_function' => array( $this, 'tag_added' ),
	 ...
	 * @param $user_id
	 */
	public function tag_added( $class, $tag_id ) {
