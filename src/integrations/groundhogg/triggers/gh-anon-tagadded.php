...
class GH_ANON_TAGADDED {
	...
	public function __construct() {
		$this->trigger_code = 'ANONGHTAGAPPLIED';
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			'sentence'            => sprintf( __( '{{A tag:%1$s}} is added to a contact', 'uncanny-automator-pro' ), $this->trigger_meta ),
			/* translators: Logged-in trigger - Groundhogg */
			'select_option_name'  => __( '{{A tag}} is added to a contact', 'uncanny-automator-pro' ),
			'action'              => 'groundhogg/contact/tag_applied',
			'priority'            => 10,
			'accepted_args'       => 2,
			'validation_function' => array( $this, 'anon_tag_added' ),
	 ...
	 * @param $tag_id
	 */
	public function anon_tag_added( $class, $tag_id ) {
		$user_id = $class->get_user_id();
		// Get all recipes that have the "$this->trigger_code = 'ANONGHTAGADDED'" trigger
