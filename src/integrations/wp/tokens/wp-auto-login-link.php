...
class WP_Auto_Login_Link {
	...
	/**
	 * This token is set up in the advanced token dropdown which is hardcoded in
	 * uncanny-automator/src/recipe-ui/src/components/Utilities.js It is only
	 * initialized if Automator Pro is active. The parser is in
	 * uncanny-automator/src/integrations/wp/tokens/wp-auto-login-token.php
	 * @see case 'AUTOLOGINLINK': This class only the auto login process once
	 *     the user uses the link
	 */
	public function __construct() {
		...
		add_filter(
			'automator_maybe_parse_AUTOLOGINLINK',
			array(
				$this,
				'generate_hash_auto_login',
			),
			99,
			4
		);
	...
	public function generate_hash_auto_login( $replaceable, $field_text, $match, $user_id ) {
		...
		if (
			false === apply_filters( 'automator_pro_auto_login_link_disable_security_check', false, $user_id ) &&
			(
