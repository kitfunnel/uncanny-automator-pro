...
/**
 * Class AMELIA_USER_APPOINTMENT_BOOKED_SERVICE_CANCELLED
 *
...
class AMELIA_USER_APPOINTMENT_BOOKED_SERVICE_CANCELLED {
	 ...
	 * @var string
	 */
	const TRIGGER_CODE = 'AMELIA_USER_APPOINTMENT_BOOKED_SERVICE_CANCELLED';
	 ...
	 * @var string
	 */
	const TRIGGER_META = 'AMELIA_USER_APPOINTMENT_BOOKED_SERVICE_CANCELLED_META';
	...
	public function setup_trigger() {
		...
		$this->add_action( 'AmeliaBookingCanceled' );
		...
		$this->set_sentence(
			...
			/* Translators: Trigger sentence */
				esc_html__( "A user's booking of an appointment for {{a specific service:%1\$s}} is canceled", 'uncanny-automator' ),
		...
		/* Translators: Trigger sentence */
			esc_html__( "A user's booking of an appointment for {{a specific service}} is canceled", 'uncanny-automator' )
		);
	...
	public function validate_trigger( ...$args ) {
		...
		$reservation = array_shift( $args );

		$reservation = isset( $reservation[0] ) ? $reservation[0] : null;

		return 'appointment' === (string) $reservation['type'] && 'canceled' === (string) $reservation['status'];
